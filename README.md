...
4. **Anti-spam de reseñas**: decidir e implementar la protección de contenido (recomendado: Cloudflare Turnstile) — deliberadamente pendiente, las reseñas hoy se publican sin moderación ni captcha. El rate limiting por IP en `src/pages/api/*` ya está implementado en código (`src/lib/rateLimit.ts`, binding `RATE_LIMITER` en `wrangler.toml`); falta confirmar que cada uno de los 7 proyectos de Cloudflare tenga ese binding activo (se define en `wrangler.toml`, verificar que el proyecto lo lea o crearlo manualmente en el dashboard si el deploy es vía Git de Pages).
